...
| `cache`           | `cache`          | optional boolean value to support selective caching of service group configurations when running with s`mongodb` and in-memory cache has been enabled. The default is `false`. When not running with a cache, this field is ignored.                   |
...
| `cache`           | `cache`          | optional Boolean value to support selective caching of device configurations when running with `mongodb` and in-memory cache has been enabled. The default is `false`.                                                                                 | `true/false`                                  |
